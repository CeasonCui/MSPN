...
class JointsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.stage == 'train':
            ...
            # print("img_id: ",img_id,"scale: ",scale,"  scale_ratio: ", scale_ratio)
            scale *= scale_ratio
        ...
        if self.stage == 'train':
            ...
            for i in range(labels_num):
                labels[i] = self.generate_heatmap(              #label是个关键点矩阵。一个图（矩阵）里关键点的位置为1的矩阵
                        joints, valid, kernel=self.gaussian_kernels[i])
            labels = torch.from_numpy(labels).float()
            # print('label_shape', labels.shape, 'valid', valid)
    ...
    def generate_heatmap(self, joints, valid, kernel=(7, 7)):
        ...
        for i in range(self.keypoint_num):
            if valid[i] < 1:         #只考虑有效关键点
                continue
            target_y = joints[i, 1] * self.output_shape[0] \
                    / self.input_shape[0]
            target_x = joints[i, 0] * self.output_shape[1] \
                    / self.input_shape[1]                       #把关键点都按比例缩小
            heatmaps[i, int(target_y), int(target_x)] = 1       #把关键点在图上标记，矩阵上对应点就是1
